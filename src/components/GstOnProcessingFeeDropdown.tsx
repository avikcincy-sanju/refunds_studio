...
const PRESET_VALUES = [0, 5, 10, 11, 12, 13, 14, 15];
...
interface GstOnProcessingFeeDropdownProps {
  value: string;
  onChange: (value: string) => void;
}

export function GstOnProcessingFeeDropdown({ value, onChange }: GstOnProcessingFeeDropdownProps) {
  const [selectedPreset, setSelectedPreset] = useState<string>(() => {
    if (value === '') return '';
    const numVal = parseFloat(value);
    const matched = PRESET_VALUES.find(v => v === numVal);
  ...
  const handlePresetChange = (e: React.ChangeEvent<HTMLSelectElement>) => {
    ...
    if (val === 'Other') { setCustomInput(''); onChange(''); return; }
  ...
  const handleCustomChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (num < 0) { setValidationError('GST/VAT/TAX on Processing Fee % cannot be negative.'); return; }
    onChange(sanitized);
  };
    ...
    <div className="bg-white border-2 border-yellow-300 rounded-2xl p-4 shadow-sm hover:border-yellow-400 hover:shadow-md transition-all">
      <div className="flex items-center justify-between mb-3">
        <div className="flex items-center gap-1.5">
          <span className="text-xs font-semibold uppercase tracking-wider text-yellow-700">GST/VAT/TAX on Processing Fee %</span>
          <div className="relative" ref={tooltipRef}>
            ...
            {showTooltip && (
              <div className="absolute right-0 top-6 z-50 w-80 bg-gray-900 text-white text-xs rounded-xl px-3 py-2.5 shadow-xl leading-relaxed">
                Select a standard GST/VAT/TAX-on-fee value or choose Other to enter a custom rate.
